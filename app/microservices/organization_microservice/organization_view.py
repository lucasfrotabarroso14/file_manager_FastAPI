...
async def create_new_organization(organization_name: str, DB_session: Session = Depends(get_session)):
    try:
        repository = OrganizationRepository(DB_session)
        new_organization, status = repository.create_organization_db(organization_name)

        if status:
            return {
                "status": True,
                "status_code": 201,
                "result": new_organization,
            }
        else:
            return {
                "status": False,
